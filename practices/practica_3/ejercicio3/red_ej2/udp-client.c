         ...
         uint16_t datalen)
{
  ...
  LOG_INFO("Temperatura en Fahrenheit calculada por el servidor= '%d' grados.", *data);
...
PROCESS_THREAD(udp_client_process, ev, data)
{
  ...
  const CelsiusDegrees_t INTERNAL_TEMPERATURE = 38;
  const CelsiusDegrees_t EXTERNAL_TEMPERATURE = 35;
  ...
  while(1) {
    ...
    if(NETSTACK_ROUTING.node_is_reachable() && NETSTACK_ROUTING.get_root_ipaddr(&dest_ipaddr)) {
      ...
      simple_udp_sendto(&udp_conn, &temperatureToSend, sizeof(temperatureToSend), &dest_ipaddr);
    } else {
