...
def create_ml_gauge_metrics():
    global prom_water_ml

    prom_water_ml = Gauge( 'water_ml',
        'Water Measure [ml]'
    )
...
def main():
    ...
    while True:
        ...
        pattern = b"msg_to_mqtt_exporter: "
        
        # Check if the received line starts with the specified pattern
        if line.startswith(pattern):
            # Publish data on corresponding topic
            client.publish(topic="water_ml", payload=line[len(pattern):], qos=0, retain=False)
